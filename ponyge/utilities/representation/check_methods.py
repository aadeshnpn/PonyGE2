...
def check_ind(parameter, ind, check):
    """
    ...
    if ind.invalid and \
            ((check == "crossover" and parameter.params['NO_CROSSOVER_INVALIDS']) or
             (check == "mutation" and parameter.params['NO_MUTATION_INVALIDS'])):
        # We have an invalid.
        return True

    elif parameter.params['MAX_TREE_DEPTH'] and ind.depth > parameter.params['MAX_TREE_DEPTH']:
        # Tree is too deep.
        return True

    elif parameter.params['MAX_TREE_NODES'] and ind.nodes > parameter.params['MAX_TREE_NODES']:
        # Tree has too many nodes.
        return True

    elif parameter.params['MAX_GENOME_LENGTH'] and len(ind.genome) > \
            parameter.params['MAX_GENOME_LENGTH']:
        # Genome is too long.
        return True


def check_genome_mapping(parameter, ind):
    """
...
def check_ind_from_parser(parameter, ind, target):
    """
    ...
    # Check the phenotype matches the target string.
        ...
        check_genome_mapping(parameter, ind)


def check_genome_from_tree(parameter, ind_tree):
    """
    ...
    if ind_tree.children:
        ...
        productions = parameter.params['BNF_GRAMMAR'].rules[ind_tree.root]['choices']
    ...
    for kid in ind_tree.children:
        ...
        check_genome_from_tree(parameter, kid)
...
def get_nodes_and_depth(parameter, tree, nodes=0, max_depth=0):
    """
...
def generate_codon(parameter, NT, choice):
    """
    ...
    productions = parameter.params['BNF_GRAMMAR'].rules[NT]
